...
int main() {
  ...
  while (getline(in_map_, line)) {
	...
	int lane = 1; // Starting ego vehicle  in lane 1
	double ref_vel = 0.0; //ref_vel to target in MPH
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
        	...
        	// Ego Vehicle's localization Info
          	double car_x = j[1]["x"];
          	double car_y = j[1]["y"];
          	double car_s = j[1]["s"];
          	double car_d = j[1]["d"];
			...
			for(int i = 0; i<sensor_fusion.size(); i++){
				...
				if(d<(2+4*lane +2) && (d>2+4*lane-2)){
					...
					check_car_s += (double)prev_size*0.02*check_speed; // Future position of ego car
						...
						if(left_lane >= 0 && left_lane <= 2){
							...
							for(int j = 0; j<sensor_fusion.size(); j++){
								...
								if(d_local <(2+4*target_lane +2) && (d_local >2+4*target_lane-2)){
									...
									check_car_s_local += (double)prev_size*0.02*check_speed_local; // Future position of car
            ...
            // check if car ahead of ego car in right lane is moving faster and car behind is at safe distance, make right lane change
						if(right_lane >= 0 && right_lane <= 2){
							bool safe_flag = true;
							int target_lane = right_lane;
							for(int j = 0; j<sensor_fusion.size(); j++){
								float d_local = sensor_fusion[j][6];
								if(d_local <(2+4*target_lane +2) && (d_local >2+4*target_lane-2)){
									double vx_local = sensor_fusion[j][3];
      ...
      // If car is too close to car in front, then slow down otherwise speed up with constant acceleration
			...
			// if previous state of the car is almost empty, use the car as a starting reference
			if(prev_size<2){
			...
			// else use the previous points as starting reference
			else{
